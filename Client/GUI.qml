...
import GUI.o3d.permissions 1.0 as Permissions
import GUI.o3d.rules 1.0 as Rules
...
RootResolutionScope {
	...
	onClosing: {
		...
		logger.debug("CentralCommand", "User " + logOut.username + " logged out from " + logOut.machineName + " monitoring the site: " + TypeHelper.uuidToString( logOut.siteId ) )
	...
	Unlimited_Security.Application {
		...
		applicationName: "Central Command"
		applicationFullName: globalActiveConfiguration && globalActiveConfiguration.customApplicationName || "Unlimited_Security 3D Central Command"
		applicationCopyright: "2006-2018 GUI"
		organizationName: "GUI"
		windowIcon: "app:CentralCommand.ico"
		...
		Permissions.FeedPermissionProviderAggregator {
			...
			property variant serviceReferences: _private.localServices.serviceFinder.sequenceNumber && _private.localServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.permissions.FeedPermissionProvider")
			property variant remoteServiceReferences: _private.remoteServices.dispatcherReady ? _private.remoteServices.serviceRegistryCache.sequenceNumber && _private.remoteServices.serviceRegistryCache.findAllServiceReferencesDataByType("GUI.o3d.permissions.FeedPermissionProvider") : null
		...
		OmniQml.ApplicationWindow {
			...
			key: "CentralCommandApplicationWindow"
			...
			Unlimited_Security.EventFilter {
				...
				//Central Command immediately after opening the investigation perspective. The crash happens
			...
			Rectangle {
				...
				InitializationSystem.InitializationUI {
					...
					Component {
						...
						CentralCommandInitializationSequence {
							autoStart: true
							...
							onCompleted: {
								logger.debug("CentralCommand", "Initialization sequence completed");
								destroy();
							}
						}
					}
				...
				Item {
							...
							StyledPopoutContainer {
								...
								SearchComboBox {
									...
									onQueryClicked: {
										...
										if (MetaObject.inherits(query, "GUI::o3d::events::StoredEventQuery")) {
											selectedPanel = "events";
											eventQueryPanelController.setQuery(query);
											eventQueryPanelController.startQuery();
										} else if (MetaObject.inherits(query, "GUI::o3d::alarms::StoredAlarmQuery")) {
											selectedPanel = "alarms";
											alarmQueryPanelController.setQuery(query);
											alarmQueryPanelController.startQuery();
										} else if (MetaObject.inherits(query, "GUI::o3d::resources::StoredResourceQuery")) {
					...
					// Display the bottom row of CentralCommand main window.
					Row {
						id: rightBottomRow
		...
		OmniWorld.FileStreamCallbackConfiguration {
			callbackProvider: OmniWorld.StreamToFileStorageAdapter {
				...
				fileStorage: MetaObject.interfaceCast(globalLocalFileStorage, "GUI::o3d::file_storage::IFileStorage")
			}
		}
		...
		Repeater {
			...
			property variant serviceReferences: _private.localServices.serviceFinder.sequenceNumber && _private.localServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.deployment.MachineService") || []
		...
		XvrService.DeviceRegistrar2Aggregator {
			...
			// Local properties
			property variant localRegistrarServiceReferences: localExposedServiceRegistryCache.sequenceNumber && localExposedServiceRegistryCache.findAllServiceReferencesDataByType("GUI.o3d.xvr_service.DeviceRegistrar2") || []
			property variant localRegistrarReferenceToRemote: ({})
			property variant localRegistrarReferenceToHealthStatus: ({})
			property variant localExporterServiceReferences: _private.localServices.serviceFinder.sequenceNumber && _private.localServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.video_clip.VideoClipExporter") || []
			...
			// Remote properties
			property variant remoteRegistrarServiceReferences: remoteMonitoringAccessTracker.accessEnabled && _private.remoteServices.serviceFinder.sequenceNumber && _private.remoteServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.xvr_service.DeviceRegistrar2") || []
			property variant remoteRegistrarReferenceToRemote: ({})
			property variant remoteRegistrarReferenceToHealthStatus: ({})
			property variant remoteExporterServiceReferences: remoteMonitoringAccessTracker.accessEnabled && _private.remoteServices.serviceFinder.sequenceNumber && _private.remoteServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.video_clip.VideoClipExporter") || []
			...
			function updateRegistrars(registrarReferences, exporterReferences, dispatcher, local) {
				...
				for (var k = 0; k < removedReferences.length; ++k) {
					...
					if (remote) {
						globalDeviceAggregator.removeRegistrar(MetaObject.interfaceCast(remote, "GUI::o3d::xvr_service::IDeviceRegistrar2"));
				...
				for (var m = 0; m < addedReferences.length; ++m) {
					...
					globalDeviceAggregator.addRegistrar(MetaObject.interfaceCast(registrar, "GUI::o3d::xvr_service::IDeviceRegistrar2"), capabilities);
			...
			function updateExporters(exporterReferences, dispatcher, local) {
					...
					if (remote) {
						...
						for (var registrar in registrarReferenceToRemotes) {
							if (registrar.search(exporterRegistrarName) !== -1 || registrar.search(exporterRegistrarName2) !== -1) {
								globalDeviceAggregator.removeVideoClipExporterFromRegistrar(MetaObject.interfaceCast(registrarReferenceToRemotes[registrar], "GUI::o3d::xvr_service::IDeviceRegistrar2"));
				...
				for (var m = 0; m < addedReferences.length; ++m) {
					...
					if (foundRegistrar) {
						...
						globalDeviceAggregator.addVideoClipExporterToRegistrar(MetaObject.interfaceCast(registrarReferenceToRemotes[foundRegistrar], "GUI::o3d::xvr_service::IDeviceRegistrar2"), MetaObject.interfaceCast(exporter, "GUI::o3d::video_clip::IVideoClipExporter"));
					}
				}
		...
		Item {
			...
			function pushEvent(newEvent) {
				var eventsVector = TypeHelper.buildVector(newEvent.toSharedPointerCast("GUI::o3d::events::Event"),"GUI::o3d::events::Event")
		...
		Unlimited_Security.FeedController {
			...
			feedManager: MetaObject.interfaceCast(theFeedManager, "GUI::o3d::xvr_service::IFeedManager")

			XvrService.XvrFeedManager {
				id: xvrFeedManager

				deviceRegistrar: MetaObject.interfaceCast(globalDeviceAggregator, "GUI::o3d::xvr_service::IDeviceRegistrar")
		...
		Unlimited_Security.AccessControlStatusManager {
			...
			property variant localServs: _private.localServices.serviceFinder.sequenceNumber && _private.localServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.access_control.AccessControlServiceDispatcher")
			property variant remoteServs: _private.remoteServices.serviceFinder.sequenceNumber && _private.remoteServices.serviceFinder.findAllServiceReferencesDataByType("GUI.o3d.access_control.AccessControlServiceDispatcher")
		...
		// We allow to launch the procedure editor from Central Command.
		...
		Events.LogToEventAppender {
			...
			iEventSink: MetaObject.interfaceCast(failsafeEventSinkDecorator, "GUI::o3d::events::IEventSink")
		...
		Item {
			...
			Component.onCompleted: {
				...
				var command = [
					"VideoRenderer/GUI.O3D.VideoRenderer.exe",
				...
				logger.debug(videoRendererFeature, "Starting Video Renderer integration with command '%1'".arg(command.join(" ")));
				videoRendererModule.start(command.join(" "));
			}
		}
		...
		Sops.ProcedureBlockTemplateRepository {
			...
			paths: packagesManager.ready ? ["imports/GUI/o3d/sops/templates"].concat(packagesManager.loadedPackagesPath || []) : []
		}
